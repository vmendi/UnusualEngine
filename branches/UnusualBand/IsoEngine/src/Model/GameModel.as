package Model
{
	...
	public class GameModel extends EventDispatcher
	{
		...
		{
			mAStartMapSpace = new AStartMapSpace(GatherIsoComponents(false), mIsoCamera.TheIsoBackground, CellSizeMeters);
		...
		private function LoadFromXML(myXML : XML) : void
		{
			...
			// Nosotros somos los ultimos en recibir el mensaje. Si hay un error, hay que tirar el CentralLoader, puesto que
			// si no, volvera a llamar a este listener. Es lo que pasa por no querer controlar todo los posibles eventos. (OK)
			mIsoEngine.TheCentralLoader.addEventListener("LoadComplete-SWF", OnLoadComplete, false, int.MIN_VALUE);

			// Hasta que no estén todos los XML importados, no mandamos a cargar los SWFs
			mIsoEngine.TheCentralLoader.addEventListener("LoadComplete-ImportXML", OnImportXMLComplete, false, int.MIN_VALUE);
			...
			function OnImportXMLComplete(event:Event):void
			{
				...
				mIsoEngine.TheCentralLoader.removeEventListener("LoadComplete-ImportXML", OnImportXMLComplete);
			...
			function OnLoadComplete(event:Event):void
			{
				...
				mIsoEngine.TheCentralLoader.removeEventListener("LoadComplete-SWF", OnLoadComplete);
		...
		private function GatherIsoComponents(forcedToBackground : Boolean) : Array
		{
			var ret : Array = new Array();
