package Model
{
	...
	public class IsoCamera extends UIComponent
	{
		...
		public function get TargetPosX() : Number { return mCamTarget.x; }
		public function set TargetPosX(val : Number):void { TargetPos = new Point(val, mCamTarget.y); }
		
		public function get TargetPosY() : Number { return mCamTarget.y; }
		public function set TargetPosY(val : Number):void { TargetPos = new Point(mCamTarget.x, val); }

		/** Factor de Zoom, medido en pixels de pantalla por metro de mundo */
		static public function get PixelsPerMeter() : Number { return mPixelsPerMeter; }
		static public function set PixelsPerMeter(n : Number) : void { mPixelsPerMeter = n; }

		/** El IsoBackground asociado a la camara */
		public function get TheIsoBackground() : IsoBackground { return mIsoBackground; }

		public function IsoCamera(isoEngine:IsoEngine)
		{
			mIsoBackground = new IsoBackground(isoEngine);
		...
		private function OnParentResize(event:Event):void
		{
			...
			// Redispachamos para que lo lean los componentes en el GameModel
			dispatchEvent(event);
		}
		...
		public function MoveWithKeyboard(elapsedTime : Number, bCtrlModifier : Boolean) : void
		{
			...
			if (KeyboardHandler.Keyb.IsKeyPressedRepeatWithOS(Keyboard.UP, bCtrlModifier))
				dirMov = dirMov.add(new Point(1, 1));
			else
			if (KeyboardHandler.Keyb.IsKeyPressedRepeatWithOS(Keyboard.DOWN, bCtrlModifier))
				dirMov = dirMov.add(new Point(-1, -1));

			if (KeyboardHandler.Keyb.IsKeyPressedRepeatWithOS(Keyboard.RIGHT, bCtrlModifier))
				dirMov = dirMov.add(new Point(1, -1));
			else
			if (KeyboardHandler.Keyb.IsKeyPressedRepeatWithOS(Keyboard.LEFT, bCtrlModifier))
				dirMov = dirMov.add(new Point(-1, 1));

			mCamVel = dirMov;
			mCamVel.normalize(MOVEMENT_SPEED);
		...
		static private const MOVEMENT_SPEED : Number = 5.0;
		...
		static private var mPixelsPerMeter : Number = 50.0;
	}
}