package Model
{
	...
	public class AssetBundle extends EventDispatcher
	{
		...
		private function GatherMovieClipNames(loader:Loader):void
		{
			...
			var defNames : Array = utils.getDefinitionNames(loader.contentLoaderInfo, false, true);
...
/* La nueva versión mientras volvemos a probar la del ruso.
private function GatherMovieClipNames(loader:Loader):void
{
	var exportMC : MovieClip = (loaderInfo.content as DisplayObjectContainer).getChildByName("mcExport") as MovieClip;

	if (exportMC == null)
		throw "mcExport not found in AssetBundle " + this.mSWFURL;

	for (var c:int=0; c < exportMC.numChildren; c++)
	{
		var childMC : MovieClip = exportMC.getChildAt(c) as MovieClip;

		if (childMC != null)
		{
			var name : String = getQualifiedClassName(childMC);

			if (mMovieClipNames.indexOf(name) != -1)
			{
    			trace("Duplicated MovieClip name: " + name + " in SWF: " + mSWFURL);
    			continue;
			}
			mMovieClipNames.push(name);
		}
	}
}
*/