package GameComponents
{
	...
	{
		virtual public function OnPreStart():void {}				/** El sistema llama antes de las OnStart() */
		virtual public function OnStart():void {}					/** El sistema llama al comenzar el juego. */
		virtual public function OnStartComplete():void {}			/** El sistema llama al final de todas las llamadas a OnStart */
		 ...
		 *
		 * Usamos esto por ejemplo para las rows del DataGrid, que tienen que estar en una colección, y
		...
		public function ReflectGameComponent() : ArrayCollection
		{
			...
			{
				// Nos deshacemos de las variables que no nos interesan
				var bIgnore : Boolean = false;
				var metadataList : XMLList = property.description.child("metadata");
				for (var c:int=0; c < metadataList.length(); c++)
				{
					if (metadataList[c].attribute("name").toXMLString() == "NonSerializable")
					{
						bIgnore = true;
						break;
					}
