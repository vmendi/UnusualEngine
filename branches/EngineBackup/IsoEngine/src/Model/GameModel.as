package Model
{
	...
	public class GameModel extends EventDispatcher
	{
		...
		public function AttachToRenderCanvas(theRenderCanvas : DisplayObjectContainer) : void
		{
			...
			// Con el cambio a Spark, no vale con un addChild (lo que nos entra tiene que ser un Group)
			(mRenderCanvas as Object).addElement(mIsoCamera);
			(mRenderCanvas as Object).addElement(mRender2DCamera);
		...
		public function RemoveFromRenderCanvas() : void
		{
			...
			(mRenderCanvas as Object).removeElement(mRender2DCamera);
			(mRenderCanvas as Object).removeElement(mIsoCamera);
			mRenderCanvas = null;
		}
		...
		public function StartGame() : void
		{
			...
			for each(sceneObj in mSceneObjs)
			...
			// Notificamos de que se han completado todos los Starts
			for each(sceneObj in mSceneObjs)
				for each(comp in sceneObj.TheAssetObject.TheGameComponents)
					comp.OnStartComplete();
		}

		private function StartSceneObject(sceneObj:SceneObject):void
		{
			// Si el SceneObj es creado durante el juego hay que mandar tb el evento de "OnPreStart"  & "StartComplete", 
			// puesto que se crea en solitario
			if (mGameStarted)
				for each(comp in sceneObj.TheAssetObject.TheGameComponents)
					comp.OnPreStart();
		 ...
		 * La pausa real se produce al acabar el frame, después de todos los updates.
		 */
		public function PauseGame(pause : Boolean) : void
		{
			...
			mPaused = pause;
			mRefreshPauseState = true;
		}

		private function RefreshPauseState() : void
		{
			if (!mRefreshPauseState)
				return;
		...
		{
			// Tenemos que refrescar siempre pq nos pueden pausar desde eventos de raton, timers, etc... No siempre se producira en OnUpdate.
			RefreshPauseState();
			...
			// El before y el after NO están pensandos para los componentes, sino para objetos externos al gameModel! Por
			// eso usamos un evento. Si quisieramos que los componentes tuvieran su before & after, usaríamos el mismo patron
			// de updatelist.
			...
			// La pausa no afecta al before & after, el update es para componentes y los otros dos sólo para el resto.
			// Por ejemplo la carga de mapa se produce en el after update, y se le hace caso aunque no estemos pausados.
			dispatchEvent(afterUpdate);

			// Si hubiera una pausa pendiente, se produciría aquí, al final de todo. 
			RefreshPauseState();
		...
		public function BroadcastMessage(msg : String, params : Object) : void
		{
			if (!mGameStarted)
				throw "Game not started, components not ready";
			
			for each(var sceneObj : SceneObject in mSceneObjs)
			{
				for each(var comp : GameComponent in sceneObj.TheAssetObject.TheGameComponents)
				{
					var clInfo : ClassInfo = new ClassInfo(comp);
					var method : MethodInfo = clInfo.method(msg);
		...
		private var mRefreshPauseState : Boolean = false;
