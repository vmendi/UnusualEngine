package Editor
{
	...
	public class LoadSaveController extends EventDispatcher
	{
		...
		public function SaveAllExternalAssetBundles():void
		{
			for each(var bundle : AssetBundle in mGameModel.TheAssetLibrary.AssetBundles)
			{
				if (bundle.ImportURL != "")
					mHelper.SaveStringToFile(bundle.GetExportXML().toXMLString(), bundle.ImportURL);
			}
		}
		...
		private var mAssetBundleController : AssetBundleController;	// Alguna funcionalidad se la delegamos a el
	}
}