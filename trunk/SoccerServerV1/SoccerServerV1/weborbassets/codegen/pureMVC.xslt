...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.service">
    ...
    <folder name="model">
        ...
        <folder name="vo">
			<xsl:for-each select="preceding-sibling::datatype|following-sibling::datatype">
				<file name="{@name}.as">
					<xsl:call-template name="codegen.description">
...
package <xsl:value-of select="//service/@namespace"/>.model.vo
{
	...
	{	<xsl:for-each select="field">
		...
		{
		return <xsl:for-each select="field">this.<xsl:value-of select="@name"/><xsl:if test="position()!=last()"> + ": " +</xsl:if>
		</xsl:for-each>;
		}
	}
}
