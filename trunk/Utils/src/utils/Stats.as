package utils
{
	...
	public class Stats extends Sprite {	
		...
		public function Stats() : void {
			...
			xml = <xml><ms>MS:</ms><msMax>MS MAX:</msMax><mem>MEM:</mem><memMax>MAX:</memMax></xml>;
			
			style = new StyleSheet();
			style.setStyle('xml', {fontSize:'9px', fontFamily:'_sans', leading:'-2px'});
			style.setStyle('fps', {color: hex2css(colors.fps)});
			style.setStyle('ms', {color: hex2css(colors.ms)});
			style.setStyle('msMax', {color: hex2css(colors.ms)});
		...
		private function update(e : Event) : void {
			...
			{
				ms_prev = timer;
				xml.msMax = "MAX MS: " + ms_max;
				ms_max = 0;
			}
			
			var elapsed : Number = timer - ms;
			
			mem = Number((System.totalMemory * 0.000000954).toFixed(3));
			mem_max = mem_max > mem ? mem_max : mem;
			ms_max = ms_max > elapsed? ms_max : elapsed;	 

			graph.scroll(-1, 0);
			graph.fillRect(rectangle, colors.bg);
			
			if (elapsed < 33)
				graph.setPixel(graph.width - 1, graph.height - ( elapsed / 3 ), colors.ms);
			else
				graph.setPixel(graph.width - 1, graph.height - ( elapsed / 3), colors.ms_warning);
			
			//graph.setPixel(graph.width - 1, graph.height - (33 / 3), colors.ms_max_for_warning);
		...
		private function update_old(e:Event):void
		{
			timer = getTimer();
			...
			if( timer - 1000 > ms_prev ) {
				...
				graph.setPixel(graph.width - 1, graph.height - ( ( timer - ms ) ), colors.ms);
...
class Colors {	
	...
	public var ms_warning : uint = 0xff0000;
	public var ms_max_for_warning : uint = 0xffff00;
