package GameComponents.Insignia
{
	...
	public final class OIInterface extends GameComponent
	{
		override public function OnStart() : void
		{
			...
			if (TheGameModel.GlobalGameState.hasOwnProperty("Code"))
				GotoPerfil(null);
			else
				GotoMain(null);
		}
		...
		private function OnLoginClickHandler(event:MouseEvent):void
		{
			...
			TheGameModel.GlobalGameState.Nick = "Zincutrin"; 
			TheGameModel.GlobalGameState.Code = "lalala";
			TheGameModel.GlobalGameState.eMail = vars.usr;
			TheGameModel.GlobalGameState.Password = vars.pass;
		}
		...
		private function OnLoginRequestComplete(event:GenericEvent):void
		{
			...
			if (result == "FOUND")
			{
				TheGameModel.GlobalGameState.Nick = responseXML.user_name; 
				TheGameModel.GlobalGameState.Code = responseXML.user_code; 
		...
		private function OnPerfilEndFrame():void
		{
			...
			TheVisualObject.ctNombre.text = TheGameModel.GlobalGameState.Nick;
		...
		private function OnRegistroRequestComplete(event:GenericEvent):void
		{
			...
			{
				ShowMessagePopup("Tu registro requiere activación. Accede a tu email y haz click en el enlace de verificación.", GotoMain);
			}
			else
		...
		private function GetURLVars(updating:Boolean) : URLVariables
		{
			...
			if (!updating)
				...
				vars.email = TheGameModel.GlobalGameState.eMail;
		...
		{
			if (TheGameModel.GlobalGameState.UnusualMode)
				return;
