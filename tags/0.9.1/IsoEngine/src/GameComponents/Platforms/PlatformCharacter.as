package GameComponents.Platforms
{
	...
	public final class PlatformCharacter extends GameComponent
	{
		...
		private function listenForKeys():void
		{
			...
			// Eje horizontal
			if ( KeyboardHandler.Keyb.IsKeyPressed(Keyboard.RIGHT) && ( !mIsCrouching || mIsCrouching && mInAir ) )
			{
				mJoystickH = 1;
				mXmov += mWalkIncrement;
				Walk();
			}
			else if ( KeyboardHandler.Keyb.IsKeyPressed(Keyboard.LEFT) && ( !mIsCrouching || mIsCrouching && mInAir ) )
			{
				mJoystickH = -1;
				mXmov -= mWalkIncrement;
				Walk();
