package GameComponents
{
	...
	/**
	 * Espacio de busqueda para el A estrella, se calcula a partir de la caminabilidad del fondo y de los propios SceneObjs del mundo.
	 */
	public class AStarMapSpace extends GameComponent implements IAStarSearchable
	{
		override public function OnStart() : void
		{
			mCellSizeInMeters = GameModel.CellSizeMeters;
			
			var isoComps : Array = TheGameModel.GatherIsoComponents(false).slice();
			var isoBackground : IsoBackground = TheGameModel.TheIsoCamera.TheIsoBackground;
		...
		public function IsNeighborWalkable(neighborOrient : int, worldPos : Point3) : Boolean
		{
			var srcPoint : IntPoint = WorldToSearchSpace(worldPos);
			
			srcPoint.x += IsoOrient.VECTORS[neighborOrient].x;
			srcPoint.y += IsoOrient.VECTORS[neighborOrient].y;
			
			var bRet : Boolean = false;
			if (IsInsideBounds(srcPoint))
				bRet = mCells[srcPoint.x + (srcPoint.y*mBoundsWidth)];
				 			
			return bRet;
		}
		
		public function IsInsideBounds(searchSpacePos : IntPoint) : Boolean
		{
			return searchSpacePos.x >= 0 && searchSpacePos.y >= 0 && searchSpacePos.x < mBoundsWidth && searchSpacePos.y < mBoundsHeight; 
		} 
